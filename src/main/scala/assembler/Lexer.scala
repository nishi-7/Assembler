...
object Lexer {
    private val keywords = Map(
        ...
        "-1" -> MinusOne,
        "!A" -> BangA, "!D" -> BangD, "!M" -> BangM,
        "-A" -> MinusA,  "-D" -> MinusD,  "-M" -> MinusM,
        "A+1" -> APlusOne, "D+1" -> DPlusOne,  "M+1" -> MPlusOne,
        "A-1" -> AMinusOne,  "D-1" -> DMinusOne,  "M-1" -> MMinusOne,
        "D+A" -> DPlusA, "D+M" -> DPlusM,
        "D-A" -> DMinusA, "D-M" -> DMinusM, "A-D" -> AMinusD, "M-D" -> MMinusD,
        "D&A" -> DAndA, "D&M" -> DAndM,
        "D|A" -> DOrA, "D|M" -> DOrM
    )
}
            ...
            case Some(c) => {
                if (this.isLetter()) {
                    ...
                    keywords.get(ident) match {
                        case Some(ty) => Token(ty, Loc(line, left - ident.length, ident.length))
                        case None => Token(Symbol(ident), Loc(line, left - ident.length, ident.length))
                    }
